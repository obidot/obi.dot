...
contract BifrostCodecWrapper {
    ...
    function encodeRedeemVToken(uint32 vCurrencyId, uint256 amount, bytes32 beneficiary)
        ...
        returns (bytes memory)
    {
        return BifrostCodec.encodeRedeemVToken(vCurrencyId, amount, beneficiary);
    }
    ...
    function encodeDEXSwap(
        ...
        return BifrostCodec.encodeDEXSwap(currencyIn, currencyOut, amountIn, amountOutMin, beneficiary);
    }
    ...
    function encodeFarmingClaim(uint256 poolId, bytes32 beneficiary) external pure returns (bytes memory) {
    ...
    function encodeSALPContribute(uint32 parachainId, uint256 amount, bytes32 beneficiary)
        ...
        returns (bytes memory)
    {
        return BifrostCodec.encodeSALPContribute(parachainId, amount, beneficiary);
    }
    ...
    function bifrostAccountDestination(bytes32 accountId) external pure returns (bytes memory) {
...
contract BifrostCodec_SLP_Test is Test {
    ...
    function test_encodeMintVToken_nonZero() public view {
        ...
        bytes memory result = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 100 ether, beneficiary);
    ...
    function test_encodeMintVToken_containsPalletIndex() public view {
        bytes32 beneficiary = keccak256("alice");
        bytes memory result = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 1 ether, beneficiary);
    ...
    function test_encodeRedeemVToken_nonZero() public view {
        ...
        bytes memory result = wrapper.encodeRedeemVToken(BifrostCodec.CURRENCY_VDOT, 50 ether, beneficiary);
    ...
    function testFuzz_encodeMintVToken_nonZeroProducesOutput(uint256 amount) public view {
        vm.assume(amount > 0);
        bytes32 beneficiary = keccak256("fuzz_user");
        bytes memory result = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, amount, beneficiary);
        assertTrue(result.length > 10, "Encoded message should have substantial length");
    }
}
...
contract BifrostCodec_DEX_Test is Test {
    ...
    function test_encodeDEXSwap_nonZero() public view {
        ...
        bytes memory result =
            wrapper.encodeDEXSwap(BifrostCodec.CURRENCY_DOT, BifrostCodec.CURRENCY_VDOT, 10 ether, 9 ether, beneficiary);
        assertTrue(result.length > 0);
        assertEq(uint8(result[0]), BifrostCodec.XCM_V4);
    }
    ...
    function test_encodeDEXSwap_containsDEXPalletIndex() public view {
        bytes32 beneficiary = keccak256("trader");
        bytes memory result =
            wrapper.encodeDEXSwap(BifrostCodec.CURRENCY_DOT, BifrostCodec.CURRENCY_BNC, 5 ether, 4 ether, beneficiary);
        bool found = false;
        for (uint256 i = 0; i < result.length; i++) {
    ...
    function testRevert_encodeDEXSwap_zeroAmountIn() public {
        bytes32 beneficiary = keccak256("trader");
        vm.expectRevert(BifrostCodec.ZeroAmount.selector);
        wrapper.encodeDEXSwap(BifrostCodec.CURRENCY_DOT, BifrostCodec.CURRENCY_VDOT, 0, 0, beneficiary);
    }
}
...
contract BifrostCodec_Farming_Test is Test {
    ...
    function test_encodeFarmingDeposit_nonZero() public view {
        ...
        bytes memory result = wrapper.encodeFarmingDeposit(0, 100 ether, beneficiary);
        assertTrue(result.length > 0);
        assertEq(uint8(result[0]), BifrostCodec.XCM_V4);
    }
    ...
    function test_encodeFarmingWithdraw_nonZero() public view {
        bytes32 beneficiary = keccak256("farmer");
        bytes memory result = wrapper.encodeFarmingWithdraw(1, 50 ether, beneficiary);
        assertTrue(result.length > 0);
    }
    ...
    function test_encodeFarmingDeposit_containsFarmingPalletIndex() public view {
        bytes32 beneficiary = keccak256("farmer");
        bytes memory result = wrapper.encodeFarmingDeposit(0, 1 ether, beneficiary);
        bool found = false;
        for (uint256 i = 0; i < result.length; i++) {
        ...
        assertTrue(found, "Encoded message should contain Farming pallet index");
    }
}
...
contract BifrostCodec_SALP_Test is Test {
    ...
    function test_encodeSALPContribute_nonZero() public view {
        ...
        bytes memory result = wrapper.encodeSALPContribute(2030, 500 ether, beneficiary);
        assertTrue(result.length > 0);
        assertEq(uint8(result[0]), BifrostCodec.XCM_V4);
    }
    ...
    function test_encodeSALPContribute_containsSALPPalletIndex() public view {
        bytes32 beneficiary = keccak256("contributor");
        bytes memory result = wrapper.encodeSALPContribute(2030, 1 ether, beneficiary);
        bool found = false;
        for (uint256 i = 0; i < result.length; i++) {
...
contract BifrostCodec_Destination_Test is Test {
    ...
    function test_bifrostDestination_deterministic() public view {
        ...
        assertEq(keccak256(dest1), keccak256(dest2), "Destination encoding should be deterministic");
    }
    ...
    function test_bifrostAccountDestination_different_accounts() public view {
        ...
        assertTrue(keccak256(destA) != keccak256(destB), "Different accounts should produce different destinations");
    }
}
...
contract BifrostCodec_CompactEncoding_Test is Test {
    ...
    function test_encodeMintVToken_smallAmount() public view {
        bytes32 beneficiary = keccak256("alice");
        bytes memory result = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 1, beneficiary);
        assertTrue(result.length > 0);
    }
    ...
    function test_encodeMintVToken_mediumAmount() public view {
        bytes32 beneficiary = keccak256("alice");
        bytes memory result = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 16000, beneficiary);
        assertTrue(result.length > 0);
    }
    ...
    function test_encodeMintVToken_largeAmount() public view {
        bytes32 beneficiary = keccak256("alice");
        bytes memory result = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 1_000_000_000, beneficiary);
        assertTrue(result.length > 0);
    }
    ...
    function test_encodeMintVToken_veryLargeAmount() public view {
        bytes32 beneficiary = keccak256("alice");
        bytes memory result = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, type(uint128).max, beneficiary);
        assertTrue(result.length > 0);
    }
    ...
    function test_encodeMintVToken_compactBoundaries() public view {
        ...
        bytes memory r1 = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 63, beneficiary);
        assertTrue(r1.length > 0);

        // Two byte mode: 64-16383
        bytes memory r2 = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 64, beneficiary);
        assertTrue(r2.length > 0);
        bytes memory r3 = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 16383, beneficiary);
        assertTrue(r3.length > 0);

        // Four byte mode: 16384 - 1073741823
        bytes memory r4 = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 16384, beneficiary);
        assertTrue(r4.length > 0);
        bytes memory r5 = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 1073741823, beneficiary);
        assertTrue(r5.length > 0);

        // Big integer mode: >= 1073741824
        bytes memory r6 = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 1073741824, beneficiary);
    ...
    function test_encodeMintVToken_differentAmounts() public view {
        bytes32 beneficiary = keccak256("alice");
        bytes memory r1 = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 100, beneficiary);
        bytes memory r2 = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 200, beneficiary);
        assertTrue(keccak256(r1) != keccak256(r2), "Different amounts should produce different encodings");
    }
    ...
    function test_encodeMintVToken_differentCurrencies() public view {
        bytes32 beneficiary = keccak256("alice");
        bytes memory r1 = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_DOT, 100, beneficiary);
        bytes memory r2 = wrapper.encodeMintVToken(BifrostCodec.CURRENCY_KSM, 100, beneficiary);
        assertTrue(keccak256(r1) != keccak256(r2), "Different currencies should produce different encodings");
    }
}
