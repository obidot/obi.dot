...
contract CrossChainCodecWrapper {
    ...
    function decodeDepositSync(bytes calldata body) external pure returns (CrossChainCodec.DepositSyncMessage memory) {
        return CrossChainCodec.decodeDepositSync(body);
    }

    function decodeWithdrawRequest(bytes calldata body)
        external
        pure
        returns (CrossChainCodec.WithdrawRequestMessage memory)
    {
        return CrossChainCodec.decodeWithdrawRequest(body);
    }

    function decodeAssetSync(bytes calldata body) external pure returns (CrossChainCodec.AssetSyncMessage memory) {
        return CrossChainCodec.decodeAssetSync(body);
    }

    function decodeStrategyReport(bytes calldata body)
        external
        pure
        returns (CrossChainCodec.StrategyReportMessage memory)
    {
        return CrossChainCodec.decodeStrategyReport(body);
    }

    function decodeEmergencySync(bytes calldata body)
        external
        pure
        returns (CrossChainCodec.EmergencySyncMessage memory)
    {
        return CrossChainCodec.decodeEmergencySync(body);
    }

    function decodeDepositAck(bytes calldata body) external pure returns (CrossChainCodec.DepositAckMessage memory) {
        return CrossChainCodec.decodeDepositAck(body);
    }

    function decodeWithdrawFulfill(bytes calldata body)
        external
        pure
        returns (CrossChainCodec.WithdrawFulfillMessage memory)
    {
...
contract CrossChainCodec_Roundtrip_Test is Test {
    ...
    function test_depositSync_roundtrip() public view {
        CrossChainCodec.DepositSyncMessage memory original = CrossChainCodec.DepositSyncMessage({
        ...
        CrossChainCodec.DepositSyncMessage memory decoded = wrapper.decodeDepositSync(encoded);
    ...
    function test_depositSync_messageTypeByte() public view {
        CrossChainCodec.DepositSyncMessage memory msg_ = CrossChainCodec.DepositSyncMessage({
            chainId: bytes("ETHEREUM"),
            depositor: address(0xdead),
            amount: 1 ether,
        ...
        assertEq(wrapper.messageType(encoded), CrossChainCodec.MSG_DEPOSIT_SYNC);
    }

    function testFuzz_depositSync_roundtrip(address depositor, uint256 amount, uint256 sharesMinted, uint256 nonce)
        public
        view
    {
        CrossChainCodec.DepositSyncMessage memory original = CrossChainCodec.DepositSyncMessage({
    ...
    function test_withdrawRequest_roundtrip() public view {
        CrossChainCodec.WithdrawRequestMessage memory original = CrossChainCodec.WithdrawRequestMessage({
        ...
        CrossChainCodec.WithdrawRequestMessage memory decoded = wrapper.decodeWithdrawRequest(encoded);
    ...
    function test_withdrawRequest_messageTypeByte() public view {
        CrossChainCodec.WithdrawRequestMessage memory msg_ = CrossChainCodec.WithdrawRequestMessage({
            chainId: bytes("BASE"),
            withdrawer: address(0xbeef),
            amount: 1 ether,
        ...
        assertEq(wrapper.messageType(encoded), CrossChainCodec.MSG_WITHDRAW_REQUEST);
    }

    // ── Asset Sync ──────────────────────────────────────────────────────

    function test_assetSync_roundtrip() public view {
        CrossChainCodec.AssetSyncMessage memory original = CrossChainCodec.AssetSyncMessage({
        ...
        CrossChainCodec.AssetSyncMessage memory decoded = wrapper.decodeAssetSync(encoded);
    ...
    function test_assetSync_messageTypeByte() public view {
        CrossChainCodec.AssetSyncMessage memory msg_ = CrossChainCodec.AssetSyncMessage({
            globalTotalAssets: 1,
            globalTotalShares: 1,
            totalRemoteAssets: 0,
            timestamp: 1
    ...
    function test_strategyReport_roundtrip() public view {
        CrossChainCodec.StrategyReportMessage memory original = CrossChainCodec.StrategyReportMessage({
            strategyId: 99,
            success: true,
        ...
        CrossChainCodec.StrategyReportMessage memory decoded = wrapper.decodeStrategyReport(encoded);
    ...
    function test_strategyReport_messageTypeByte() public view {
        CrossChainCodec.StrategyReportMessage memory msg_ = CrossChainCodec.StrategyReportMessage({
            strategyId: 0,
            success: true,
            returnedAmount: 0,
            pnl: 0,
        ...
        assertEq(wrapper.messageType(encoded), CrossChainCodec.MSG_STRATEGY_REPORT);
    }

    // ── Emergency Sync ──────────────────────────────────────────────────

    function test_emergencySync_roundtrip() public view {
        CrossChainCodec.EmergencySyncMessage memory original = CrossChainCodec.EmergencySyncMessage({
            paused: true,
            emergencyMode: true,
        ...
        CrossChainCodec.EmergencySyncMessage memory decoded = wrapper.decodeEmergencySync(encoded);
    ...
    function test_emergencySync_emptyReason() public view {
        CrossChainCodec.EmergencySyncMessage memory original =
            CrossChainCodec.EmergencySyncMessage({paused: false, emergencyMode: false, reason: bytes("")});
    ...
    function test_emergencySync_messageTypeByte() public view {
        CrossChainCodec.EmergencySyncMessage memory msg_ =
            CrossChainCodec.EmergencySyncMessage({paused: true, emergencyMode: false, reason: bytes("")});
        bytes memory encoded = CrossChainCodec.encodeEmergencySync(msg_);
        assertEq(wrapper.messageType(encoded), CrossChainCodec.MSG_EMERGENCY_SYNC);
    }

    // ── Deposit Ack ─────────────────────────────────────────────────────

    function test_depositAck_roundtrip() public view {
        CrossChainCodec.DepositAckMessage memory original =
            CrossChainCodec.DepositAckMessage({depositNonce: 42, globalTotalAssets: 10_000_000 ether, accepted: true});

        bytes memory encoded = CrossChainCodec.encodeDepositAck(original);
        CrossChainCodec.DepositAckMessage memory decoded = wrapper.decodeDepositAck(encoded);
    ...
    function test_depositAck_rejected() public view {
        CrossChainCodec.DepositAckMessage memory original =
            CrossChainCodec.DepositAckMessage({depositNonce: 0, globalTotalAssets: 0, accepted: false});
    ...
    function test_depositAck_messageTypeByte() public view {
        CrossChainCodec.DepositAckMessage memory msg_ =
            CrossChainCodec.DepositAckMessage({depositNonce: 0, globalTotalAssets: 0, accepted: true});
    ...
    function test_withdrawFulfill_roundtrip() public view {
        CrossChainCodec.WithdrawFulfillMessage memory original =
            CrossChainCodec.WithdrawFulfillMessage({withdrawNonce: 3, amount: 25 ether, fullyFulfilled: true});

        bytes memory encoded = CrossChainCodec.encodeWithdrawFulfill(original);
        CrossChainCodec.WithdrawFulfillMessage memory decoded = wrapper.decodeWithdrawFulfill(encoded);
    ...
    function test_withdrawFulfill_partial() public view {
        CrossChainCodec.WithdrawFulfillMessage memory original =
            CrossChainCodec.WithdrawFulfillMessage({withdrawNonce: 1, amount: 10 ether, fullyFulfilled: false});
    ...
    function test_withdrawFulfill_messageTypeByte() public view {
        CrossChainCodec.WithdrawFulfillMessage memory msg_ =
            CrossChainCodec.WithdrawFulfillMessage({withdrawNonce: 0, amount: 0, fullyFulfilled: true});
        bytes memory encoded = CrossChainCodec.encodeWithdrawFulfill(msg_);
        assertEq(wrapper.messageType(encoded), CrossChainCodec.MSG_WITHDRAW_FULFILL);
    }
}
...
contract CrossChainCodec_Error_Test is Test {
    ...
    function testRevert_messageType_emptyBody() public {
        vm.expectRevert(abi.encodeWithSelector(CrossChainCodec.MessageTooShort.selector, 0, 1));
    ...
    function testRevert_decodeDepositSync_tooShort() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(CrossChainCodec.MessageTooShort.selector, 20, 33));
    ...
    function testRevert_decodeAssetSync_tooShort() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(CrossChainCodec.MessageTooShort.selector, 10, 33));
    ...
    function testRevert_decodeWithdrawRequest_tooShort() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(CrossChainCodec.MessageTooShort.selector, 5, 33));
    ...
    function testRevert_decodeStrategyReport_tooShort() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(CrossChainCodec.MessageTooShort.selector, 32, 33));
    ...
    function testRevert_decodeEmergencySync_tooShort() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(CrossChainCodec.MessageTooShort.selector, 1, 33));
    ...
    function testRevert_decodeDepositAck_tooShort() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(CrossChainCodec.MessageTooShort.selector, 16, 33));
    ...
    function testRevert_decodeWithdrawFulfill_tooShort() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(CrossChainCodec.MessageTooShort.selector, 2, 33));
    ...
    function test_messageTypeConstants_unique() public pure {
        ...
        for (uint256 i = 0; i < types.length; i++) {
            for (uint256 j = i + 1; j < types.length; j++) {
                assertTrue(types[i] != types[j], "Message type constants must be unique");
            }
        }
    }
