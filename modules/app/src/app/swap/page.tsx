...
import type { SwapRouteResult, TradeActionType } from "@/types";
...
export default function SwapPage() {
  ...
  const handleInputChange = useCallback(
    (params: {
      ...
      const tokenPairChanged =
        !prev ||
        prev.tokenIn !== params.tokenIn ||
        prev.tokenOut !== params.tokenOut;
  ...
  const activeDescription =
    TRADE_ACTIONS.find((t: { id: string }) => t.id === activeTab)
      ?.description ?? "";
  ...
  return (
    <div className="space-y-5">
      <PageHero
        eyebrow="Trade"
        title="Standalone Swap"
        description="Legacy standalone trading surface kept visually aligned with the routed trade experience."
      />
      <InfoBanners />
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[420px_minmax(0,1fr)]">
        ...
        <div className="panel overflow-hidden flex flex-col">
          {/* Tab bar */}
          <div className="border-b-[3px] border-border bg-surface-alt px-4 pt-4 pb-3">
            <div className="tab-group">
              {TRADE_ACTIONS.map(
                (action: { id: TradeActionType; label: string }) => (
                  <button
                    key={action.id}
                    type="button"
        ...
        <div className={cn("panel hidden overflow-hidden lg:flex lg:flex-col")}>
          {showDiagram ? (
            <RouteDiagram
            ...
            <div className="flex flex-1 flex-col items-center justify-center gap-3 p-8 text-center">
              ...
              <div className="relative">
                <div className="flex h-12 w-12 items-center justify-center border-[3px] border-border bg-primary/10 shadow-[3px_3px_0_0_var(--border)]">
                ...
                <div className="flex flex-wrap justify-center gap-2 mt-2">
                  {routes.adapters
                    ...
                    .map((a) => (
                      <span
                        key={a.label}
                        className="pill bg-surface-hover text-[0.8rem] text-text-muted"
                      >
                        {a.label}
                      </span>
                    ))}
      ...
      {showDiagram && (
        <div className="panel mt-3 lg:hidden">
          <RouteDiagram
            tokenIn={swapInput.tokenIn}
            tokenOut={swapInput.tokenOut}
            amountIn={swapInput.amountIn}
