...
export function UserPosition() {
  ...
  if (!isConnected) {
    return (
      <div className="border-t-[3px] border-border bg-surface px-4 py-4">
        <p className="retro-label mb-1 text-[0.85rem] text-text-muted">
          My Position
        </p>
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 border-t-[3px] border-border bg-surface px-4 py-4">
  ...
  if (!position || position.sharesBalance === 0n) {
    return (
      <div className="border-t-[3px] border-border bg-surface px-4 py-4">
        ...
        <p className="text-[12px] text-text-muted">
          No vault shares. Deposit tDOT above to start.
        </p>
      </div>
    );
  }

  const sharesFormatted = formatTokenAmount(
    position.sharesBalance.toString(),
    18,
    6,
  );
  const valueFormatted = formatTokenAmount(
    position.assetsValue.toString(),
    18,
    6,
  );
  ...
  return (
    <div className="border-t-[3px] border-border bg-surface px-4 py-4">
      <p className="retro-label mb-3 text-[0.85rem] text-text-muted">
        My Position
      </p>
